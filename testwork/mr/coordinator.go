...
func (c *Coordinator) Getonejob(args *ExampleArgs, reply *RequestArgs) error {
	for {
		...
		default:
			select {
			case <-Alldone:
				//joballdone,so pass a wrong job with out of jobtype range
				reply.OneJob = Job{JobType: 5}
				reply.Mapnum = 0
				reply.Reducenum = 0
				return nil
			default:
				//jobchan has no job,should wait job be putted
...
func (c *Coordinator) HandleReport() {
	for {
		select {
		case <-Alldone:
			fmt.Printf("all jobs done,thread Handlereport exit\n")
		default:
			//do nothing
		}
	...
	for {
		...
		if jobshouldputted != nil {
			...
			if finishedJobNum == c.JOBmap_num && jobtype == JOBtypeMap {
				//切换到reduce
				close(MapjobAlldone)
				fmt.Printf("all map jobs done,thread putjob exit\n")
				return
				//exit
			}
			if finishedJobNum == c.JOBreduce_num && jobtype == JOBtypeReduce {
				//广播任务结束
				close(Alldone)
				fmt.Printf("all jobs done,thread putjob exit\n")
				return
				//exit
			}
		}
...
func (c *Coordinator) Checktimeoutjob() {
	for {
		...
		case <-Alldone:
			//exit this thread
			fmt.Printf("all jobs done,thread Checktimeoutjob exit\n")
			return
		default:
			//do nothing
		}
		c.Heaplock.Lock()
		if c.JOBheap.Len() != 0 && time.Now().Unix()-c.JOBheap[0].JobTime > 20 {
			//超时
			c.Maplock.Lock()
			//
			if c.JOBall_map[c.JOBheap[0].JobID].JobState == JOBwaiting {
			...
			c.Maplock.Unlock()
			heap.Pop(&c.JOBheap)
			fmt.Printf("pop job,heap has %d job\n", c.JOBheap.Len())
		}
		c.Heaplock.Unlock()
...
func (c *Coordinator) Addjobtoheap(job *Job) {
	...
	fmt.Printf("add job,heap has %d job\n", c.JOBheap.Len())
}
