...
func (wp *WorkerParameter) DoReduceWork(reducef func(string, []string) string) error {
	...
	for idx := 0; idx < wp.Mapnum; idx++ {
		//wp.WorkJob.JobID-wp.Mapnum : because in coordinator's map ,reducejob index is not equal to its id
		filename := fmt.Sprintf("mr-%d-%d", idx, wp.WorkJob.JobID-wp.Mapnum)
	...
	outFileName := fmt.Sprintf("mr-out-%d", wp.WorkJob.JobID-wp.Mapnum)
...
func (wp *WorkerParameter) Requestonejob() {
	...
	fmt.Printf("get jobid %d file name %s \n", wp.WorkJob.JobID, wp.WorkJob.File)
}
...
func Worker(mapf func(string, string) []KeyValue,
	...
	for flag {
		...
		default:
			//wrong jobtype(set 5)
			fmt.Printf("worker got closed signal\n")
			return
		}
	}
}
